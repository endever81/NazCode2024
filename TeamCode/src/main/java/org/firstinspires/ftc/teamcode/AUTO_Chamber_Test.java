...
@Config
@Autonomous(name = "Auto Chamber Full Test", group = "Autonomous")
public class AUTO_Chamber_Test extends LinearOpMode {
    public class Lift {
    ...
    {     ElapsedTime runtime = new ElapsedTime();
        ...
        if (opModeIsActive()) {
            ...
            newLiftTarget = robot.cascadeLift.getCurrentPosition() + (int) (inches * (1140/(3.5 * 3.1415)));

            robot.cascadeLift.setTargetPosition(newLiftTarget);

            robot.cascadeLift.setMode(DcMotor.RunMode.RUN_TO_POSITION);

            runtime.reset();
            robot.cascadeLift.setPower(Math.abs(power));
    ...
    {     ElapsedTime runtime = new ElapsedTime();
        ...
        if (opModeIsActive()) {
            ...
            //newLiftTarget = robot.Extender.getCurrentPosition() + (int) (inches * (1140/(3.5 * 3.1415)));

            //robot.Extender.setTargetPosition(newLiftTarget);

            //robot.Extender.setMode(DcMotor.RunMode.RUN_TO_POSITION);

            runtime.reset();
            //robot.Extender.setPower(Math.abs(power));
